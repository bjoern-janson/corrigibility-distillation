"""Corrected CPython opcode instrumentation for RG-002."""
...
class OpcodeCounterV2:
    def __init__(self, codes: Iterable[CodeType]):
        self.codes = frozenset(codes)
        self.count = 0

    def trace(self, frame, event, arg):
        if frame.f_code in self.codes and event in ("call", "line"):
    ...
    def run(self, fn: Callable[[], object]) -> tuple[object, int]:
        ...
        if previous is not None:
            raise RuntimeError("RG-002 requires no pre-existing Python trace function")
        try:
...
def count_cost_v2(*, build, predict, build_codes, predict_codes, nested_build_codes) -> dict[str, int]:
    build_counter = OpcodeCounterV2(tuple(build_codes) + tuple(nested_build_codes))
    state, construction = build_counter.run(build)

    prediction_counter = OpcodeCounterV2(tuple(predict_codes))
